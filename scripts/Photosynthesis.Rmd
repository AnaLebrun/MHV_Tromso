...
Temperature_plot<-ggplot(hour_temperature,aes(x=date))+
  ...
  theme_fira()+
  scale_x_datetime(minor_breaks = "day",breaks=seq(as.POSIXct("2022-06-30 12:46:11 CET"),as.POSIXct("2022-07-25 07:00:03 CET"), by="48 hours"),date_labels = "%m-%d")+
...
## 3° Diving PAM <br>
```{r install chatpgt, echo=FALSE, warning=FALSE, message=FALSE, results=FALSE,fig.width=15,fig.height=10}
library(chatgpt)
Sys.setenv(OPENAI_API_KEY = "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")
ask_chatgpt("J'ai des données de quantum yield dans le dataframe Kelp_Pam le 02/07, 07/07, 11/07, 14/07, 18/07 et 22/07 pour 3 espèces de macroalgues (AE, LD et SL) ayant subis 4 conditions différentes : contrôle (C), high temperature (HT), 1 marine heatwaves (1MH) et 2 marine heatwaves (2MH). et répartis dans 12 mésocosmes différents donnant ainsi un triplicata de chaque conditions : 3 mésocosmes par conditions. Quelles analyse statistiques puis-je faire pour analyser la différence statistique du quantum yield en fonction du temps et des conditions subis par chaque espèce ? Ecris moi le code")

Kelp_Pam(complete_code("Une fonction qui analyse la différence statistique du quantum yiled (Fv/Fm) en fonction du temps et des conditons subis par chaque espèce AE, LD et SL"))
chatgpt::ask_chatgpt()
...
Kelp_Pam_alongtime_plot_no_outlier<-ggplot(Kelp_PAM_no_outlier,aes(y=`Fv/Fm`,x=date_modified,color=treatment_renamed))+
  ...
  scale_color_manual(values=c("lightgrey","#92c5de","#E48121","#FC1D28","#901C21"))+
  theme_fira()+
  #stat_summary(aes(y=0.50,x=date_modified),colour="black",data=Kelp_PAM_no_outlier,fun.data = give.n, geom = "text",size=8)+
...
$$ <br> with V = Volume of the chamber, in L (V=30L for the big chamber and V=XXL for the small chamber) <br>
...
Week1b$O2_normalized<-Week1b$estimate*30/Week1b$Dry_weight
Week1b$variance<-Week1b$std.error^2
Week1b$variance<-Week1b$variance*30/Week1b$Dry_weight
...
ggplot(O2,aes(y=O2_normalized,x=date_modified,color=treatment))+
  geom_jitter(shape = 16,size=3,alpha=0.8)+
  geom_boxplot(aes(group=grp,fill=treatment),outlier.alpha = 0)+
  facet_wrap(species~.,ncol=1)+
  scale_fill_manual(values=c("lightgrey","#404040","#92c5de","#E48121","#FC1D28","#901C21"))+
  scale_color_manual(values=c("lightgrey","#404040","#92c5de","#E48121","#FC1D28","#901C21"))+
  stat_summary(aes(y=5,x=date_modified),colour="black",data=O2,fun.data = give.n, geom = "text",size=8)+
  theme_fira()+
  scale_x_continuous(label=c("07-02","07-05","07-10","07-12","07-14","07-18","07-21 & 22"),breaks=c(0,0.75,2,2.5,3,4,5))+
  scale_y_continuous(limits=c(0.0,6),breaks=c(0,1,2,3,4,5),expand = expansion(mult = c(0,0)))+
  theme(axis.title = element_text(size = rel(1.5), hjust=0.5),
        axis.text = element_text(size = rel(2)),
        strip.text.x = element_text(size = rel(2)),
        panel.border = element_rect(colour = "black", fill=NA),
...
rosnerTest(O2_Alaria_T0$O2_normalized, k = 1)$all.stats # no outlier : 0.0623768186
...
rosnerTest(O2_Alaria_10$O2_normalized, k = 1)$all.stats # 1 outlier
O2<-O2[!O2$O2_normalized>=0.06,]
O2_Alaria<-O2[O2$species=="Alaria",]
O2_Alaria_10<-O2_Alaria[O2_Alaria$week=="2022-07-10",]
...
rosnerTest(O2_Saccharina_T0$O2_normalized, k = 1)$all.stats
...
O2<-O2[!(O2$O2_normalized>=0.03&O2$week=="2022-07-05"),]
O2_Saccharina<-O2[O2$species=="Saccharina",]
O2_Saccharina_05<-O2_Saccharina[O2_Saccharina$week=="2022-07-05",]
...
O2_Alaria<-O2[O2$species=="Alaria",]
O2_hlm<-lmer(O2_normalized ~ week*treatment + (1|ID), data=O2_Alaria)
Anova(O2_hlm)
...
AIC(O2_hlm)
AIC(O2_glm)
# The glm has the lowest AIC, it's therefore the best model.
rm(O2_hlm) # remove the 2nd model (the worst one)

# significative differences:
#(2022-07-10 1MH) - TF 1MH            1.841907 0.382 64   4.822  0.0039
#(2022-07-10 1MH) - TF 2MH            1.736914 0.382 64   4.547  0.0097
#(2022-07-10 1MH) - TF C              1.831604 0.382 64   4.795  0.0043
#(2022-07-10 1MH) - (2022-07-14 HT)   1.927778 0.382 64   5.047  0.0018


O2_Saccharina<-O2[O2$species=="Saccharina",]
O2_hlm<-lmer(O2_normalized ~ week*treatment + (1|ID), data=O2_Saccharina)
Anova(O2_hlm)
