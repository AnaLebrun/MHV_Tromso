...
library(nlme)
#length_lm <- lme(Growth_rate ~ `Total lenght (cm)`, weights = ~species,data=Growth_normalized)
length_lm <- lm(Growth_rate ~ `Total lenght (cm)`,data=Growth_normalized)
plot(Growth_rate ~ `Total lenght (cm)`, data=Growth_normalized)
abline(length_lm)
summary(length_lm)
# equation : y=0.2163884 + (-0.0011954)*total_lenght + 0.09646
# with y = growth rate
Growth_normalized$Normalized_growth_rate<-(0.2163884 + (-0.0011954)*Growth_normalized$`Total lenght (cm)` + 0.09646)
ggplot(Growth_normalized,aes(x=treatment,fill=treatment))+
  #geom_bar(aes(y=mean_growth),stat='identity',color="black", position=position_dodge()) +
  #geom_errorbar(aes(ymin=mean_growth-sd_growth, ymax=mean_growth+sd_growth),width=.2,position=position_dodge(.9))+
...
length.lm.std <- residuals(length_lm, type="pearson")
shapiro.test(length.lm.std)
qqnorm(length.lm.std)
qqline(length.lm.std)
hist(length.lm.std, xlab="standardized residuals")

# homogeneity of model variances
plot(y = length.lm.std, x = data$variable1)
...
Corraline_growth_plot<-ggplot(Corraline_growth,aes(x=treatment,fill=treatment))+
  #geom_bar(aes(y=mean_growth),stat='identity',color="black", position=position_dodge()) +
  #geom_errorbar(aes(ymin=mean_growth-sd_growth, ymax=mean_growth+sd_growth),width=.2,position=position_dodge(.9))+
  geom_boxplot(aes(y=Delta_BW),outlier.alpha = 0)+
  geom_jitter(shape = 21,aes(y=Delta_BW,fill=treatment),color="black",size=3)+
  scale_fill_manual(values=c("#cccccc","#92c5de","#E48121","#FC1D28","#901C21"))+
...
# 3 outliers : -42.69, -39.48, -19.37
ggqqplot(Corraline_growth$Delta_BW)
...
rosnerTest(Corraline_growth$Delta_BW, k = 8)$all.stats # 8 outliers
shapiro.test(Corraline_growth$Delta_BW) # normal
# Plots  ####
give.n <- function(x){
   return(c(y = mean(x), label = length(x)))
}
...
ggplot(snails_growth,aes(x=treatment,fill=treatment))+
  #geom_bar(aes(y=mean_growth),stat='identity',color="black", position=position_dodge()) +
  #geom_errorbar(aes(ymin=mean_growth-sd_growth, ymax=mean_growth+sd_growth),width=.2,position=position_dodge(.9))+
  geom_boxplot(aes(y=Delta_BW),outlier.alpha = 0)+
  geom_jitter(shape = 21,aes(y=Delta_BW,fill=treatment),color="black",size=3)+
